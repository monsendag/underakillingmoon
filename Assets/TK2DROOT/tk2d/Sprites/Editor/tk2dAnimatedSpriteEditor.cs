...
class tk2dAnimatedSpriteEditor : tk2dSpriteEditor
{
	...
	void Init()
	{
		if (!initialized)
		{
			animLibs = tk2dEditorUtility.FindPrefabsInProjectWithComponent<tk2dSpriteAnimation>();
			if (animLibs != null)
			{
    ...
    static void DoCreateSpriteObject()
    {
		...
		if (sprColl == null)
		{
			...
			if (data == null)
			{
				EditorUtility.DisplayDialog("Create Animated Sprite", "Unable to create sprite as no SpriteCollections have been found.", "Ok");
				return;
			}
		...
		tk2dSpriteAnimation[] anims = tk2dEditorUtility.FindPrefabsInProjectWithComponent<tk2dSpriteAnimation>();
		if (anims == null || anims[0].clips.Length == 0)
		{
		...
		sprite.collection = anims[0].clips[0].frames[0].spriteCollection;
		sprite.renderer.material = sprite.collection.material;
		sprite.Build();
		
		sprite.spriteId = anims[0].clips[0].frames[0].spriteId;
		sprite.anim = anims[0];
    }
}
