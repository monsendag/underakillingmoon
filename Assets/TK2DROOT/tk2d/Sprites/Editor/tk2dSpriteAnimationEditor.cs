...
class tk2dSpriteAnimationEditor : Editor
{
	...
	tk2dSpriteCollection[] allSpriteCollections = null;
	...
	void InitializeInspector()
	{
		if (!initialized)
		{
			allSpriteCollections = tk2dEditorUtility.FindPrefabsInProjectWithComponent<tk2dSpriteCollection>();
			if (allSpriteCollections != null)
			{
				allSpriteCollectionNames = new string[allSpriteCollections.Length];
				allSpriteCollectionSpriteNames = new List<string[]>();
				
				for (int i = 0; i < allSpriteCollections.Length; ++i)
				{
					allSpriteCollectionNames[i] = allSpriteCollections[i].name;
					string[] spriteNames = new string[allSpriteCollections[i].textureRefs.Length];
					
					if (allSpriteCollections[i].textureRefs.Length != allSpriteCollections[i].textureParams.Length)
					{
						EditorUtility.DisplayDialog("Fatal error", "You need to Commit your spriteCollection before setting up animations", "Ok");
						allSpriteCollections = null;
						allSpriteCollectionNames = null;
						allSpriteCollectionSpriteNames = null;
						return;
					}
					
					for (int j = 0; j < allSpriteCollections[i].textureRefs.Length; ++j)
					{
						spriteNames[j] = allSpriteCollections[i].textureParams[j].name;
					}
	...
	int GetSpriteCollectionId(tk2dSpriteCollectionData data)
	{
		for (int i = 0; i < allSpriteCollections.Length; ++i)
		{
			if (allSpriteCollections[i].spriteCollection == data)
				return i;
		}
		return 0; // default
	}
    ...
    public override void OnInspectorGUI()
    {
		...
		if (allSpriteCollections == null || allSpriteCollectionNames == null || allSpriteCollectionSpriteNames == null)
		{
		...
		else // has anim clips
		{
			...
			if (GUILayout.Button("+", GUILayout.MaxWidth(28), GUILayout.MaxHeight(14)))
			{
				...
				frame.spriteCollection = allSpriteCollections[0].spriteCollection;
				frame.spriteId = 0;
			...
			if (newFrameCount != clipNumFrames)
			{
				...
				else
				{
					...
					{
						frames[i] = new tk2dSpriteAnimationFrame();
						frames[i].spriteCollection = allSpriteCollections[0].spriteCollection;
						frames[i].spriteId = 0;
					}
				}
			...
			if (horizontalAnimDisplay)
			{
				...
				for (int i = 0; i < clipNumFrames; ++i)
				{
					...
					EditorGUILayout.BeginVertical();
					{
						...
						{
							clip.frames[i].spriteCollection = allSpriteCollections[newSpriteCollectionId].spriteCollection;
							clip.frames[i].spriteId = 0;
						}
	...
	void DrawSpritePreview(int collectionId, int spriteId)
	{
		...
		Texture2D tex = tk2dSpriteCollectionEditor.GetThumbnailTexture(allSpriteCollections[collectionId], spriteId);
		if (tex)
		{
